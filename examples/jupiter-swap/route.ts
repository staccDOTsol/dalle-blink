...
app.openapi(
  ...
  async (c) => {
    ...
    const transaction = await program.methods.buy((new BN(Number(Math.floor(Number(amount)/ (candlestickData[candlestickData.length-1].close )/10**9)))), new BN(maxSolCost)).accounts({
      global,
      feeRecipient,
      mint: mintPublicKey,
...
app.openapi(
  ...
  async (c) => {
    ...
    const transaction = await program.methods.sell(new BN((Number(Math.floor(Number(amount) / (candlestickData[candlestickData.length-1].close) / 10 ** 9)))), new BN(0)).accounts({
      global,
      feeRecipient,
      mint: mintPublicKey,
      bondingCurve: bondingCurvePublicKey,
